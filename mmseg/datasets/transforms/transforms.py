...
class RandomCrop(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        img_size = img.shape[:2]
        for key in results.get('seg_fields', []):
            mask = self.crop(results[key], crop_bbox)
            if mask.shape[:2] != img_size:
                mask = mmcv.imresize(mask, img_size, interpolation='nearest')
            results[key] = mask

        results['img'] = img
        results['img_shape'] = img_size
        return results
...
@TRANSFORMS.register_module()
class ADSI(BaseTransform):
    """This class implements the ADSI transform.
    ...
    def fourier_transform(self, img):
        """Parform a Fourier transform.
        """

        f_xy = np.asarray(img)
        f_uv = np.fft.fft2(f_xy, axes=(0, 1))
        shifted_f_uv = np.fft.fftshift(f_uv, axes=(0, 1))

        return shifted_f_uv
    
    def inverse_fourier_transform(self, shifted_f_uv):
        """Parform a inverse Fourier transform.
        """

        unshifted_f_uv = np.fft.ifftshift(shifted_f_uv, axes=(0, 1))
        i_f_xy = np.fft.ifft2(unshifted_f_uv, axes=(0, 1))
        img = np.real(i_f_xy)

        return img

    def distance_based_attenuation(self, shape, center, beta, order):
        """
        Generates an attenuation mask based on distance within a box.
        
        Args:
            shape (tuple): The height and width of the image (h, w).
            center (tuple): The center coordinates of the mask (center_h, center_w).
            beta (float): The radius of the box (in both height and width directions).
            order (int): The order (N) of the Butterworth filter.

        Returns:
            ndarray: An attenuation mask of shape (h, w).
        """
        h, w = shape
        ...
        # Compute Butterworth attenuation 
        attenuation = 1 / (1 + (distance / (beta + 1e-8))**(2 * order))

        return attenuation

    def transform(self, results: dict) -> dict:
        """
        Applies a transformation to attenuate low-frequency components in the input image.

        Args:
            results (dict): A dictionary containing the input image and related data.

        Returns:
            dict: The results after applying the transformation.
        """
        ...
        shifted_f_uv = self.fourier_transform(img)

        # Set the center and size of the mask
        center_h, center_w = h // 2, w // 2
        beta_min = int(self.beta_min * min(h, w) // 2)
        beta_max = int(self.beta_max * min(h, w) // 2)
        
        beta = random.randint(beta_min, beta_max) if beta_min != beta_max else beta_min

        # Create attenuation mask based on distance
        attenuation_mask = self.distance_based_attenuation(
            shape=(h, w),
        ...
        # Apply the attenuation mask
        mixed_f_uv = shifted_f_uv * (1 - alpha * attenuation_mask)  

        # Perform inverse Fourier transform to reconstruct image
        img = self.inverse_fourier_transform(mixed_f_uv)

        # Scale the image using min-max normalization to 0–255 range
        if img.shape[:2] != (w, h):
            img = mmcv.imresize(img, (w, h), interpolation='bilinear')
        img = np.clip(img, 0, 255).astype(np.uint8)

        # Replace the image in the results with the transformed one
        results['img'] = img
